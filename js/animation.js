...
function addSDMap(){
	//17地市图层
	var gLayer = L.graphicsLayer({id:"SDLAYER"}).addTo(map);
	for ( var i = 0; i < mapContent.MapContent.length; i++) {
		var geo = mapContent.MapContent[i].geometry.rings;
		//黑色边线，不填充
		L.graphic(L.polygon(geo),L.simpleFillSymbol({fill:false,opacity:0.4,weight:1,color: '#0'})).addTo(gLayer);
	}
}
//API 示例代码
function appShow(){
...
//	var imagePoint = L.graphic(L.mapPoint(120.28125,36.54495),L.pictureMarkerSymbol({image:"./images/i_globe.png",width:20,height:30}));
	...
	var imagePoint2 = L.graphic(L.mapPoint(120.58125,36.54495),L.pictureMarkerSymbol({image:"./images/i_globe.png",width:20,height:20}));
	var context = this;
	var anim = null,anim2=null;
	function updown(up){
		// 缓动方式
...
//			easing = "Circ.easeOut";//Linear
	...
	var index = 0;
	function downup(up){	
//		y = 36.54495 + index * 0.001;
//		index ++;
//		imagePoint2.getGeometry().setY(y);
//		imagePoint2.redraw();
//		L.Util.requestAnimFrame(loop);
		...
		var easing = "Linear";
		// 动画类
		anim2 = L.animation(0,1,function(value,end){
			// 具体动作
			var y,w;
			if(up){
				y = 36.54495 + value * 1;
			...
			imagePoint2.getGeometry().setY(y);
			imagePoint2.getSymbol().options.width = w;
			imagePoint2.getSymbol().options.height = w;
			// 地洞Graphic绑定的提示窗
			imagePoint2.moveTooltip();
			// 移动Graphic绑定的弹窗
			imagePoint2.movePopup();
			// 修改弹窗内容
			imagePoint2.bindPopup("curTime:"+Date.now());
...
//			gLayer.removeAll();
	...
	imagePoint2.addTo(gLayer).on({
		click: function (){
//			anim2 = L.Util.requestAnimFrame(function (){
//				imagePoint2.setSymbol(L.simpleMarkerSymbol({style:"circle",width:20,height:100}));
//			}, this);
//			gLayer.removeAll();
			if(anim2){
				if(anim2.isRunning())
...
//			L.Util.requestAnimFrame(loop,context);
		}
	});
